"""
Smart Campus Delivery - Setup Automation Module

Automation helpers for provisioning ROS 2 Foxy and TurtleBot3 dependencies.
This module automatically prepares the entire environment for campus delivery operations:
- Installs ROS2 and TurtleBot3 packages
- Sets up workspace and dependencies  
- Configures environment variables
- One-command setup for the delivery robot system
"""
...
def automate_full_setup(
    ...
    *,
    ...
    """
    Execute the full campus delivery environment provisioning pipeline.
    
    This function prepares the entire ROS2 workspace for the smart campus
    delivery robot system with one command.
    """
    ensure_apt_repositories(dry_run=dry_run)
    ...
    if not dry_run:
        _LOG.info("Campus delivery setup complete!")
        _LOG.info("Next: run 'colcon build' inside %s before sourcing setup.bash", workspace_path)
