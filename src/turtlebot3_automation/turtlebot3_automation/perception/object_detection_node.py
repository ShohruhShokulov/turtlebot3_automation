"""
Campus Delivery Object Detection System

YOLO-powered obstacle detection for safe campus navigation.
Detects campus-specific obstacles: person, bike, box
Publishes detections to ROS 2 topics for obstacle avoidance.
"""
...
class CampusObstacleDetectionNode(Node):
    """
    Smart Campus Delivery - Obstacle Detection System
    
    Detects obstacles on campus delivery routes:
    - person: Students/faculty blocking pathways
    - bicycle: Bikes that may obstruct route  
    - box: Other packages or large obstacles
    
    Subscribes to camera frames, runs YOLO, broadcasts detections for safety.
    """

    def __init__(self) -> None:
        super().__init__("campus_obstacle_detection")

        self.declare_parameter("camera_topic", "/camera/color/image_raw")
        self.declare_parameter("model_path", "yolov8n.pt")
        self.declare_parameter("confidence_threshold", 0.4)
        # Campus-specific obstacle detection: person, bicycle, box (backpack/suitcase as proxy)
        self.declare_parameter("class_filter", ["person", "bicycle", "backpack", "suitcase"])
        self.declare_parameter("inference_period", 0.2)
        self.declare_parameter("marker_namespace", "campus_obstacles")
        ...
        self._det_pub = self.create_publisher(Detection2DArray, "campus_delivery/detections", 10)
        self._marker_pub = self.create_publisher(MarkerArray, "campus_delivery/obstacle_markers", 10)
        self._label_pub = self.create_publisher(String, "campus_delivery/obstacle_alerts", 10)

        self._image_sub = self.create_subscription(Image, camera_topic, self._on_image, 10)
        self._logger.info("=== Campus Obstacle Detection System ===")
        self._logger.info(f"Model: {model_path}")
        self._logger.info(f"Detecting: person, bicycle, box (packages)")
        self._logger.info("Monitoring campus pathways for safe delivery navigation")
    ...
    def _color_for_class(class_id: int) -> tuple:
        ...
        return CampusObstacleDetectionNode._hsv_to_rgb(hue, 0.7, 0.95)
...
def main() -> None:
    rclpy.init()
    node = CampusObstacleDetectionNode()
    try:
        rclpy.spin(node)
    except KeyboardInterrupt:  # pragma: no cover
        node.get_logger().info("Campus Obstacle Detection stopped by user.")
    finally:
        node.destroy_node()
        rclpy.shutdown()
